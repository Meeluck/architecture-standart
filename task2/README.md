...
| F4  | Оператор кол-центра после получения заявки с сайта должен иметь возможность просмотреть заявку и предложить клиенту особые условия по депозиту | |
| F5  | После подачи заявки с сайта клиент должен пройти очную идентификацию в отделении | |
| F6  | Интернет-банк должен отображать список доступных депозитов с актуальными и персонализированными ставками| |
| F7  | Интернет-банк должен позволять клиенту подать заявку на открытие депозита с указанием счёта и суммы депозита | |
| F8  | Подача заявки через интернет-банк должна подтверждаться СМС-кодом | |
| F9  | На этапе MVP заявка должна обрабатываться менеджером бэк-офиса, который подтверждает условия депозита в АБС ||
| F10  | Клиент должен получать СМС-уведомления после подтверждения ставки и после открытия депозита | |
| F11 | Решение должно обеспечивать ведение ставок по депозитам и согласование ставок по заявкам в системном виде вместо использования XLS-файлов как основного рабочего инструмента. | |
| F12 | Сотрудники бэк-офиса депозитов и бэк-офиса кредитов должны иметь возможность просматривать и использовать данные по ставкам в пределах своих бизнес-процессов и с учётом разграничения доступа | |
| | | |
...
| | | |
| +S  | + Tребования безопасности (Security)       |
...
| **Res4**  | АБС можно рассматривать только как вертикально масштабируемый компонент; требования по горизонтальному масштабированию на неё не распространяются |Прямо следует из условия: «АБС может масштабироваться только вертикально из-за своей базы данных». |
| **Res5**  | Текущая инфраструктура интернет-банка не обеспечивает требуемую отказоустойчивость и должна рассматриваться как ограничение MVP | Прямо следует из условия: «Команда интернет-банка утверждает, что эти требования пока невыполнимы». |
| **Res6**  | При проектировании нужно учитывать наличие основного и резервного ЦОД и возможность распределения нагрузки между ними в будущем | Прямо следует из условия: «У банка есть резервный ЦОД...» и «лучше предусмотреть ... распределение запросов ... между ... ЦОД». |
